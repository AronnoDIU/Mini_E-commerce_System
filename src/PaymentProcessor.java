...
public class PaymentProcessor {
    ...
    private static void validatePaymentDetails(String creditCardNumber, String expirationDate, String cvv) {
        ...
        if (creditCardNumber.length() != 16) {
            throw new IllegalArgumentException("Invalid credit card number. It must be 16 digits.");
        }

        // Validate expiration date
        String[] dateParts = expirationDate.split("/");
        if (dateParts.length != 2) {
            throw new IllegalArgumentException("Invalid expiration date format. It must be in the format MM/YYYY.");
        }
        ...
        if (month < 1 || month > 12) {
            throw new IllegalArgumentException("Invalid month in expiration date. It must be between 1 and 12.");
        }

        if (year < 2021) {
            throw new IllegalArgumentException("Invalid year in expiration date. It must be 2021 or later.");
        }
